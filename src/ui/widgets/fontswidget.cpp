    ...
    , mTextInput(nullptr)
{
    ...
    if (mToolbar) {
        ...
        mMainLayout->setContentsMargins(4, 4, 4, 0);
    ...
    setDisabled(true);
    setMaximumHeight(qRound(eSizesUI::widget * 3.0));
...
{
    if (!mTextInput || !mTextInput->isVisible()) { return; }
    mTextInput->setFocus();
...
void FontsWidget::setTextInputVisible(bool visible)
{
    if (!mTextInput) { return; }
    mTextInput->setVisible(visible);
    updateGeometry();
}

bool FontsWidget::textInputVisible() const
{
    return mTextInput && mTextInput->isVisible();
}
