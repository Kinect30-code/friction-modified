...
void Canvas::mouseDoubleClickEvent(const eMouseEvent &e) {
    ...
    if(mHoveredPoint_d) {
        ...
        if (const auto linkCanvas = enve_cast<InternalLinkCanvas*>(mHoveredBox)) {
            if (const auto targetScene = enve_cast<Canvas*>(linkCanvas->getFinalTarget())) {
                Document::sInstance->setActiveScene(targetScene);
                updateHovered(e);
                return;
            }
        } else if(enve_cast<ContainerBox*>(mHoveredBox) && !mHoveredBox->isLink()) {
