...
void VideoFrameLoader::convertFrame() {
    ...
    const AVPixelFormat srcFormat = static_cast<AVPixelFormat>(mFrameToConvert->format);
    const auto srcFmtDesc = av_pix_fmt_desc_get(srcFormat);
    const bool srcHasAlpha = srcFmtDesc && (srcFmtDesc->flags & AV_PIX_FMT_FLAG_ALPHA);
...
void VideoFrameLoader::setFrameToConvert(
    ...
    const auto srcFormat = frame && frame->format != AV_PIX_FMT_NONE
