...
void VideoStreamsData::open(const char * const path)
{
    ...
    // For VP8/VP9, try to use libvpx decoder for better alpha support
    if (vidCodecPars->codec_id == AV_CODEC_ID_VP8) {
        vidCodec = avcodec_find_decoder_by_name("libvpx");
        if (!vidCodec) vidCodec = avcodec_find_decoder(vidCodecPars->codec_id);
    } else if (vidCodecPars->codec_id == AV_CODEC_ID_VP9) {
        vidCodec = avcodec_find_decoder_by_name("libvpx-vp9");
        if (!vidCodec) vidCodec = avcodec_find_decoder(vidCodecPars->codec_id);
    }
    ...
    const bool srcHasAlpha = av_pix_fmt_desc_get(srcFormat) &&
                             (av_pix_fmt_desc_get(srcFormat)->flags & AV_PIX_FMT_FLAG_ALPHA);
    
    // For alpha formats, we need to ensure proper alpha channel handling
    const int swsFlags = srcHasAlpha ? (SWS_BICUBIC | SWS_FULL_CHR_H_INP) : SWS_BICUBIC;
    
    // Set sws parameters for better alpha handling
    const double swsParam[2] = {0.0, 0.0};
    ...
    fSwsContext = sws_getContext(fCodecContext->width,
                                 ...
                                 fCodecContext->width,
                                 fCodecContext->height,
                                 AV_PIX_FMT_RGBA, swsFlags,
                                 nullptr, nullptr, swsParam);
