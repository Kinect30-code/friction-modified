...
{
    const QString dir = getAppTempDir();
    if (dir.isEmpty() || filename.isEmpty()) { return dir; }
    return QString::fromUtf8("%1/%2").arg(dir, filename);
}

const QString AppSupport::getAppTempDir()
{
#ifdef Q_OS_LINUX
    if (isFlatpak()) {
...
#if defined(Q_OS_UNIX) && !defined(Q_OS_DARWIN)
    // 回退 fork 行为：不强制 xcb_egl，让 Qt 自动选择 GL 集成
    // (上游 "Drop GLX support" 强制 xcb_egl，导致部分驱动下画布黑屏)
