...
bool swtIsEffectCollectionTarget(const SingleWidgetTarget * const target) {
...
bool swtHasEffectAncestor(const SWT_Abstraction * const abstraction) {
    auto parent = abstraction->getParent();
    while(parent) {
        const auto target = parent->getTarget();
        if(enve_cast<eEffect*>(target) || swtIsEffectCollectionTarget(target)) {
            return true;
        }
...
bool swtShouldParticipateInViewMode(const SWT_Abstraction * const abstraction,
                                    const SWT_RulesCollection &rules,
                                    const bool satisfiesRule) {
    if(rules.fViewMode != SWT_ViewMode::effectsOnly) {
        return satisfiesRule;
    }
    if(abstraction->isMainTarget()) {
        return true;
    }
    const auto target = abstraction->getTarget();
    if(swtIsEffectCollectionTarget(target) || enve_cast<eEffect*>(target)) {
        return true;
    }
    if(swtHasEffectAncestor(abstraction)) {
        return true;
    }
    return false;
}

bool swtShouldDisplayOwnRow(const SWT_Abstraction * const abstraction,
                            const SWT_RulesCollection &rules,
                            const bool satisfiesRule,
                            const bool flattenHierarchy) {
    if(!satisfiesRule || abstraction->isMainTarget() || flattenHierarchy) {
        return false;
    }
    if(rules.fViewMode != SWT_ViewMode::effectsOnly) {
        return true;
...
bool SWT_Abstraction::setAbstractions(
    ...
    const bool baseSatisfiesRule = mTarget_k->SWT_shouldBeVisible(
    ...
    const bool childrenVisible = (satisfiesRule &&
                                  (mContentVisible || flattenHierarchy)) ||
                                 mIsMainTarget;
