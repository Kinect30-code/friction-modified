...
void AnimationBox::toggleFrameRemappingAction()
{
    if (frameRemappingEnabled()) {
        disableFrameRemappingAction();
    } else {
        enableFrameRemappingAction();
    }
}

bool AnimationBox::freezeFrameAtAbsFrameAction(int absFrame)
{
    if (!mSrcFramesCache) {
        return false;
    }
    ...
    const auto relRange = durRect->getRelFrameRange();
    const int relFrame = prp_absFrameToRelFrame(absFrame);
    if (!relRange.inRange(relFrame)) {
        return false;
    }
    ...
    if (!mFrameRemapping || !mFrameRemapping->enabled()) {
        return false;
    }

    const int frozenSourceFrame = getAnimationFrameForRelFrame(relFrame);
    const int outAbsFrame = prp_relFrameToAbsFrame(relRange.fMax);

    mFrameRemapping->anim_removeKeys({relFrame + 1, FrameRange::EMAX}, true);
    mFrameRemapping->saveValueToKey(absFrame, frozenSourceFrame);
    mFrameRemapping->saveValueToKey(outAbsFrame, frozenSourceFrame);
    mFrameRemapping->anim_setRecordingWithoutChangingKeys(true);
    return true;
}

bool AnimationBox::frameRemappingEnabled() const
{
    return mFrameRemapping && mFrameRemapping->enabled();
}

IntFrameRemapping *AnimationBox::getFrameRemapping() const
{
    return mFrameRemapping.get();
}
