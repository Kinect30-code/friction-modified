...
Property* firstEditableVectorPathProperty(SmartVectorPath* const vectorPath) {
    if(!vectorPath) {
        return nullptr;
    }
    auto* const paths = vectorPath->getPathAnimator();
    if(!paths) {
        return nullptr;
    }
...
void focusConvertedVectorPath(Canvas* const scene,
                              SmartVectorPath* const vectorPath) {
    if(!scene || !vectorPath) {
        return;
    }
    scene->clearBoxesSelection();
    scene->addBoxToSelection(vectorPath);
...
void configureConvertedVectorPath(PathBox* const source,
    ...
    vectorPath->prp_setDrawingOnCanvasEnabled(true);
    if(auto* const paths = vectorPath->getPathAnimator()) {
...
void PathBox::prp_setupTreeViewMenu(PropertyMenu * const menu) {
    if(!enve_cast<SmartVectorPath*>(this)) {
        menu->addPlainAction(
                    QIcon::fromTheme("draw-bezier-curves"),
                    tr("Convert To Path"),
                    [this]() {
            auto* const scene = getParentScene();
            if(!scene) {
                return;
            }
            if(const auto parent = getParentGroup()) {
                const auto parentName = parent->prp_getName();
                if(parentName == QStringLiteral("Masks") ||
                   parentName == QStringLiteral("__AE_LAYER_MASKS__")) {
                    return;
                }
            }

            auto* const vectorPath = objectToVectorPathBox();
            if(!vectorPath) {
                return;
            }
            const QString sourceName = prp_getName();
            configureConvertedVectorPath(this, vectorPath);
            removeFromParent_k();
            vectorPath->prp_setName(sourceName);
            focusConvertedVectorPath(scene, vectorPath);
...
SmartVectorPath *PathBox::objectToVectorPathBox() {
    ...
    auto* const parent = getParentGroup();
    if(!parent) return nullptr;
    const auto newPath = enve::make_shared<SmartVectorPath>();
    const auto relFrame = anim_getCurrentRelFrame();
    auto sourcePath = getRelativePath(relFrame);
    if(sourcePath.isEmpty()) {
        sourcePath = mEditPathSk;
    }
    newPath->loadSkPath(sourcePath);
    copyBoundingBoxDataTo(newPath.get());
    newPath->duplicateFillSettingsFrom(getFillSettings());
    newPath->duplicateStrokeSettingsFrom(getStrokeSettings());
    const int index = parent->getContainedIndex(this);
    parent->insertContained(index >= 0 ? index : 0, newPath);
    return newPath.get();
}
