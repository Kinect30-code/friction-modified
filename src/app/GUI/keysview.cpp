...
KeysView::SelectedKeySpan KeysView::getSelectedKeySpan() const
{
    SelectedKeySpan span;
    int minAbsFrame = FrameRange::EMAX;
    int maxAbsFrame = FrameRange::EMIN;
    for (const auto& anim : mSelectedKeysAnimators) {
        if (!anim) { continue; }
        const int animMin = anim->anim_getLowestAbsFrameForSelectedKey();
        const int animMax = anim->anim_getHighestAbsFrameForSelectedKey();
        if (animMin == FrameRange::EMAX || animMax == FrameRange::EMIN) {
            continue;
        }
        minAbsFrame = qMin(minAbsFrame, animMin);
        maxAbsFrame = qMax(maxAbsFrame, animMax);
    }
    if (minAbsFrame == FrameRange::EMAX || maxAbsFrame == FrameRange::EMIN) {
        return span;
    }
    span.valid = true;
    span.minAbsFrame = minAbsFrame;
    span.maxAbsFrame = maxAbsFrame;
    return span;
}

bool KeysView::prepareEdgeScaleFromPressedKey()
{
    if (!mLastPressedKey || !mLastPressedKey->isSelected()) { return false; }
    const auto span = getSelectedKeySpan();
    if (!span.valid || span.minAbsFrame >= span.maxAbsFrame) { return false; }

    const int pressedAbsFrame = mLastPressedKey->getAbsFrame();
    if (pressedAbsFrame == span.minAbsFrame) {
        mEdgeScalingKeys = true;
        mEdgeScaleStartAbsFrame = span.minAbsFrame;
        mEdgeScalePivotAbsFrame = span.maxAbsFrame;
        return true;
    }
    if (pressedAbsFrame == span.maxAbsFrame) {
        mEdgeScalingKeys = true;
        mEdgeScaleStartAbsFrame = span.maxAbsFrame;
        mEdgeScalePivotAbsFrame = span.minAbsFrame;
        return true;
    }
    return false;
}
    ...
    if(e->button() == Qt::MiddleButton) {
        ...
        if(mGraphViewed) graphMousePress(posU);
            ...
            if(!mLastPressedKey) {
                ...
                const bool edgeScaleFromSelection =
                        !shiftPressed &&
                        mLastPressedKey->isSelected() &&
                        prepareEdgeScaleFromPressedKey();
        ...
        if (mMovingKeys) {
            ...
            if (!movable) {
                if (auto *timeline = qobject_cast<TimelineWidget*>(parentWidget())) {
                    timeline->showAeTimelineContextMenu(e->globalPos());
                    return;
                }
                ...
                const QString freezeStr = tr("Freeze Frame");
                const auto animBox = enve_cast<AnimationBox*>(movable->getParentProperty());
                const QString remapStr = animBox && animBox->frameRemappingEnabled()
                        ? tr("Disable Time Remapping")
                        : tr("Enable Time Remapping");
                menu.addAction(QIcon::fromTheme("sequence"), editStr);
                menu.addAction(QIcon::fromTheme("cut"), splitStr);
                if (animBox) {
                    menu.addAction(QIcon::fromTheme("keyframe"), remapStr);
                    menu.addAction(QIcon::fromTheme("media-playback-pause"), freezeStr);
                }
                ...
                if (selectedAction) {
                    ...
                    } else if (animBox && selectedAction->text() == remapStr) {
                        animBox->toggleFrameRemappingAction();
                        if (animBox->frameRemappingEnabled()) {
                            if (auto *timeline = qobject_cast<TimelineWidget*>(parentWidget())) {
                                timeline->revealSelectedFrameRemapping();
                            }
                        }
                    } else if (animBox && selectedAction->text() == freezeStr) {
                        if (auto *window = MainWindow::sGetInstance()) {
                            if (auto *dock = window->getTimeLineWidget()) {
                                dock->freezeSelectedFramesAtCurrentTime();
                            }
                        }
                    }
                }
...
void KeysView::paintEvent(QPaintEvent *) {
    ...
    if (!mGraphViewed && mCurrentScene) {
        const auto frameIn = mCurrentScene->getFrameIn();
        const auto frameOut = mCurrentScene->getFrameOut();
        const qreal inX = frameIn.enabled ?
                    (frameIn.frame - mMinViewedFrame - 0.5)*mPixelsPerFrame :
                    0.;
        const qreal outX = frameOut.enabled ?
                    (frameOut.frame - mMinViewedFrame + 0.5)*mPixelsPerFrame :
                    width();
        if (frameIn.enabled && inX > 0.) {
            p.fillRect(QRectF(0., 0., inX, height()),
                       ThemeSupport::getThemeBaseDarkerColor(135));
        }
        if (frameOut.enabled && outX < width()) {
            p.fillRect(QRectF(outX, 0., width() - outX, height()),
                       ThemeSupport::getThemeBaseDarkerColor(135));
        }
        if (frameIn.enabled || frameOut.enabled) {
            const qreal activeLeft = frameIn.enabled ? qMax(0., inX) : 0.;
            const qreal activeRight = frameOut.enabled ? qMin<qreal>(width(), outX) : width();
            if (activeRight > activeLeft) {
                p.fillRect(QRectF(activeLeft, 0., activeRight - activeLeft, 3.),
        ...
        if(mGraphViewed && ctrlPt) {
            ...
            if(mScalingKeys) {
                ...
                if(mValueInput.inputEnabled()) {
                    ...
                    if (mEdgeScalingKeys) {
                        const qreal currentAbsFrame = xToFrame(posU.x());
                        const qreal startDistance =
                                mEdgeScaleStartAbsFrame - mEdgeScalePivotAbsFrame;
                        if (qFuzzyIsNull(startDistance)) {
                            keysScale = 1.;
                        } else {
                            keysScale = (currentAbsFrame - mEdgeScalePivotAbsFrame)/
                                        startDistance;
                            if (keysScale < 0.) { keysScale = 0.; }
                        }
                    } else {
                ...
                const int absFrame = mEdgeScalingKeys ?
                            mEdgeScalePivotAbsFrame :
...
//                                    dDFrame);
    ...
    if(mIsMouseGrabbing) Document::sInstance->scheduleInteractiveScenesUpdate();
    else update();
    mLastMovePos = pos;
}
